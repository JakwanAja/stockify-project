...
@section('page-subtitle', 'Input dan pantau transaksi barang masuk')
...
<div class="space-y-6">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-sm font-semibold text-gray-800 mb-4">Form Transaksi Masuk Baru</h3>

        <form method="POST" action="{{ route('staff.transaksi-masuk.store') }}">
            @csrf
                    ...
                    <select id="product_id" name="product_id"
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
                                {{ $product->name }} — SKU: {{ $product->sku }} (Stok: {{ $product->current_stock }})
                            </option>
                        @endforeach
                    </select>
                    @error('product_id')
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="px-5 py-4 border-b border-gray-200">
            ...
            <p class="text-xs text-gray-400 mt-0.5">Transaksi masuk yang pernah kamu ajukan</p>
        </div>
                    ...
                    @forelse($transactions as $index => $trx)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-5 py-3 font-medium text-gray-800">
                                {{ $trx->product->name ?? '—' }}
                                <p class="text-xs text-gray-400 font-normal">{{ $trx->product->sku ?? '' }}</p>
                            </td>
                            <td class="px-5 py-3 text-center font-semibold text-gray-800">
                                +{{ $trx->quantity }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center text-gray-400 text-sm">
                                Belum ada transaksi masuk yang diajukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
