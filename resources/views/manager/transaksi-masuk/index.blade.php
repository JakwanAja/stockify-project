...
@section('page-title', 'Transaksi Masuk')
@section('page-subtitle', 'Kelola konfirmasi barang masuk gudang')
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">Transaksi masuk yang perlu ditindaklanjuti</p>
            </div>
            @if($pending->count() > 0)
                            ...
                            <td class="px-5 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form id="konfirmasi-{{ $trx->id }}"
                                          method="POST"
                                          action="{{ route('manager.transaksi-masuk.konfirmasi', $trx->id) }}">
                                        @csrf
                                    </form>
                                    <button onclick="showKonfirmasiModal('konfirmasi-{{ $trx->id }}', '{{ addslashes($trx->product->name ?? '') }}', {{ $trx->quantity }})"
                                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-green-600 bg-green-50 hover:bg-green-100 rounded-lg transition">
                                    ...
                                    <form id="tolak-{{ $trx->id }}"
                                          method="POST"
                                          action="{{ route('manager.transaksi-masuk.tolak', $trx->id) }}">
                                        @csrf
                                    </form>
...
<div id="modal-konfirmasi"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        ...
        <div class="text-center mb-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-1">Konfirmasi Penerimaan</h3>
            <p class="text-sm text-gray-500">
                Terima transaksi masuk untuk
                <span id="konfirmasi-product-name" class="font-semibold text-gray-800"></span>
                sebanyak
                <span id="konfirmasi-qty" class="font-semibold text-green-600"></span> unit?
            </p>
            <p class="text-xs text-blue-500 mt-2">Stok produk akan otomatis bertambah setelah dikonfirmasi.</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <button onclick="submitKonfirmasiForm()"
                    class="flex-1 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition">
                Ya, Terima
            </button>
        </div>
...
<script>
    ...
    // ===== Modal Konfirmasi Terima =====
    function showKonfirmasiModal(formId, productName, qty) {
